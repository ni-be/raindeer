...
class TestDataHelper(unittest.TestCase):
    ...
    def setUp(self):
        self.dwd_data_valid = ["precipitation"]  # A valid DWD data to be tested
        self.interval_valid = ["annual", "monthly"]  # A valid interval to be tested
        self.option_valid = "w"  # A valid option to be tested
    ...
    def test_data_helper_valid(self):
        ...
        result = data_helper(self.dwd_data_valid, self.interval_valid, self.option_valid)
    ...
    def test_data_helper_invalid(self):
        ...
        with self.assertRaises(AssertionError):
            data_helper(self.dwd_data_invalid, self.interval_valid, self.option_valid)
            
        with self.assertRaises(AssertionError):
            data_helper(self.dwd_data_valid, self.interval_invalid, self.option_valid)
            
        with self.assertRaises(AssertionError):
            data_helper(self.dwd_data_valid, self.interval_valid, self.option_invalid)
