...
class CLITestCase(unittest.TestCase):
    ...
    def test_fourier_mode(self):
        ...
        command = ['python', 'raindeer/main.py', 'precipitation', '--mode=fourier', '-b="Hessen" "Deutschland"']
        result = subprocess.run(command, capture_output=True, text=True)
        self.assertEqual(result.returncode, 1)  # Ensure the command executed successfully
        # Add assertions for the expected output or behavior
        # Additional assertions for DataFrame output
    ...
    def test_plot_between_years(self):
        ...
        command = ['python', 'raindeer/main.py', 'precipitation', '--mode=fourier', '-b="Deutschland"' '-w=precipitation', '-y=2000..2001']
        result = subprocess.run(command, capture_output=True, text=True)
        self.assertEqual(result.returncode, 1)  # Ensure the command executed successfully
        # Add assertions for the expected output or behavior
        # Additional assertions for DataFrame output
    ...
    def test_dataframe_helper_mode(self):
        ...
        command = ['python', 'raindeer/main.py', '-dt=precipitation','-i=annual', '-m=january', '--mode=dataframe_helper']
        result = subprocess.run(command, capture_output=True, text=True)
        self.assertEqual(result.returncode, 2)  # Ensure the command executed successfully
        # Add assertions for the expected output or behavior
        # Additional assertions for DataFrame output
        first_line = result.stdout.strip().split('\n')[0]
        expected_first_line = ''
    ...
    def test_plot_between_years(self):
        ...
        command = ['python', 'raindeer/main.py', '"data/annual"', '--mode=simple-plot', '-y=2000', '-m=january', '-b="Brandenburg"', '-w="precipitation"']
        result = subprocess.run(command, capture_output=True, text=True)
        self.assertEqual(result.returncode, 1)  # Ensure the command executed successfully
        # Add assertions for the expected output or behavior
        # Additional assertions for DataFrame output
